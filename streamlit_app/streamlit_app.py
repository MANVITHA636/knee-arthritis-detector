...
# ---------------- CUSTOM GRAD-CAM (no OpenCV, no pytorch_grad_cam library) ----------------
...
def overlay_heatmap(rgb_img, cam, alpha=0.45):
    # rgb_img: float array in [0,1], shape (H, W, 3)
    heatmap = cm.jet(cam)[:, :, :3]  # RGBA -> RGB, values in [0,1]
...
# ================= STEP 3: RESULT =================
    ...
    sat_channel = np.array(hsv_img)[:, :, 1]
    mean_saturation = sat_channel.mean()

    is_valid = (
        confidence >= CONFIDENCE_THRESHOLD
        and margin >= MARGIN_THRESHOLD
        and mean_saturation >= SATURATION_THRESHOLD
    )
    ...
    if not is_valid:
        ...
        visualization = None
        try:
            cam = compute_gradcam(model, input_tensor, model.layer4[-1], pred_idx)
            visualization = overlay_heatmap(rgb_img, cam)
        except Exception:
            visualization = None
